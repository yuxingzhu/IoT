  ...
  <div>
    <div class="app-container">
        ...
        <el-row>
          <el-col :span="20" class="table-filter">
            <el-form ref="filterForm" :inline="true" :model="listQuery" label-width="0px" class="filterForm" @submit.native.prevent>
              ...
              <el-form-item>
                <el-tooltip  content="新增(从模板)">
                  <el-button class="filter-item" icon="el-icon-folder-add" @click="copyModeEditDialog()" />
                </el-tooltip>
              </el-form-item>
              <el-form-item>
              ...
              <el-form-item>
                <el-select v-model.trim="listQuery.deviceStatus" style="width: 110px" placeholder="在线状态">
                  <el-option label="在线" value="true" />
                  <el-option label="离线" value="false" />
                </el-select>
              </el-form-item>
              <el-form-item>
                <el-select v-model.trim="listQuery.runOnce" style="width: 110px" placeholder="任务状态">
              ...
              <el-form-item>
                ...
                <el-tooltip content="批量删除">
                  <el-button icon="el-icon-s-release" :disabled="abled" @click="deleteAll" />
                </el-tooltip>

                <el-tooltip content="批量迁移">
                  <el-button icon="el-icon-s-unfold" :disabled="abled" @click="pasteAll" />
                </el-tooltip>
              </el-form-item>
            </el-form>
          </el-col>
      ...
      <el-table ref="multipleTable" v-loading="listLoading" :data="list" stripe size="mini" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="设备名称" prop="deviceName" width="200" align="center" />
        ...
        <el-table-column label="设备描述" :show-overflow-tooltip="true" width="150" prop="deviceDesc" align="center">
        </el-table-column>
        <el-table-column label="数据模型" prop="profileName" width="200" align="center">
          <template scope="scope">
            <span>{{ scope.row.profileName }}</span>
          </template>
        </el-table-column>
        <el-table-column label="连接协议" prop="connectProtocol" width="150" align="center" />
        <el-table-column label="设备地址" :show-overflow-tooltip="true" prop="endPoint" align="center">
        </el-table-column>
        <el-table-column v-if="type == 1" label="在线状态" prop=" deviceStatus" width="70" align="center">
        ...
        <el-table-column label="设备状态" prop="showAdminState" width="70" align="center">
          <template slot-scope="scope">
            <svg-icon :icon-class="scope.row.showAdminState ? 'unlock' : 'lock'" />
            {{ scope.row.adminState === "UNLOCKED" ? "启用" : "停用" }}
          </template>
        </el-table-column>
        <el-table-column label="任务状态" prop="runOnce" width="70" align="center">
          <template slot-scope="scope">
            <svg-icon :icon-class="scope.row.runOnce ? 'run' : 'stop'" />
            {{ scope.row.runOnce ? "启用" : "停用" }}
          </template>
        </el-table-column>
        <el-table-column label="任务执行频率" prop="translate" width="100" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" align="center" width="550">
          <template slot-scope="scope">
            ...
            <el-tooltip  content="采集任务配置" placement="bottom" style="margin-left: 1px">
              <el-button size="small" icon="el-icon-setting" @click="task(scope.row)" class="elbuttondefault"/>
            </el-tooltip>
            ...
            <el-tooltip  content="删除" placement="bottom" style="margin-left: 1px">
              <el-button size="small" icon="el-icon-delete" @click="deletePartner(scope.row)" class="elbuttondefault"/>
            </el-tooltip>
            <el-tooltip  content="更多操作" placement="bottom" style="margin-left: 1px">
              <el-dropdown @command="handlePartnerCmd(arguments[0], scope.row)">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item command="updateDesc">修改设备描述</el-dropdown-item>
      ...
      <el-dialog :title="editDialogTitle" :visible.sync="editDialogVisible" :close-on-click-modal="false" @close="closeEditDialog">
        <div>
          <el-form ref="feDeviceAddressEditForm" :model="feDeviceAddressEditForm" :rules="feDeviceAddressEditRules" label-width="100px" size="mini" @submit.native.prevent>
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="设备编码" prop="deviceEncoding">
                  <el-col :span="22">
                    <el-input v-model.trim="feDeviceAddressEditForm.deviceEncoding" :maxlength="200" />
                  </el-col>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="设备位置" prop="deviceLocations">
                  <el-col :span="22">
              ...
              <el-col :span="20">
                <el-form-item label="设备描述" prop="deviceDesc">
                  <el-col :span="21" style="margin-left: 0px">
                    <el-input v-model.trim="feDeviceAddressEditForm.deviceDesc" type="textarea" :maxlength="20" :rows="5" />
                  </el-col>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <el-dialog :title="editDialogTitle" :visible.sync="viewVisible" style="width: 150%; margin-left: -19%" :close-on-click-modal="false" @close="closeDialog">
        <div>
          <json-viewer :value="jsonData" :expand-depth="5" boxed />
        </div>
        <div slot="footer">
      ...
      <el-dialog :title="editDialogTitle" :visible.sync="OpenCopyDialogVisible" :close-on-click-modal="false" @close="closeCopyDialog">
        <div>
          <el-form ref="copyDeviceEditForm" :model="copyDeviceEditForm" :rules="copyDeviceEditFormRule" label-width="100px" size="mini" @submit.native.prevent>
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="新设备描述" prop="deviceDesc">
                  <el-col :span="21" style="margin-left: 0px">
                    <el-input v-model.trim="copyDeviceEditForm.deviceDesc" type="textarea" :maxlength="20" :rows="5" />
                  </el-col>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <el-dialog :title="editDialogTitle" :visible.sync="editDialogDesc" :close-on-click-modal="false" @close="closeEditDialogDesc">
        <div>
          <el-form ref="feEdgeDeviceEditDesc" :model="feEdgeDeviceEditDesc" :rules="copyDeviceEditFormDesc" label-width="100px" size="mini" @submit.native.prevent>
            <el-row>
              <el-col :span="20">
                <el-form-item label="设备描述" prop="deviceDesc">
                  <el-col :span="21" style="margin-left: 0px">
                    <el-input v-model.trim="feEdgeDeviceEditDesc.deviceDesc" type="textarea" :maxlength="20" :rows="5" />
                  </el-col>
                </el-form-item>
              </el-col>
            </el-row>
        ...
        <div slot="footer">
          <el-button icon="fa-ban" @click="closeEditDialogDesc">取 消</el-button>
          <el-button type="primary" icon="fa-save" :loading="submiting" @click="saveFeDeviceDesc">确 定
          </el-button>
        </div>
      </el-dialog>
            ...
            <el-row style="margin-left: 40px; margin-top: 0px">
              ...
              <el-col v-show="
                  protocol === 'BECKHOFF_ADSNET' ||
                    ...
                    protocol === 'OPCDA' ||
                    protocol === 'MODBUS_TCP'
                " :span="20" style="margin-left: 0px; margin-top: 0px">
                <template>
          ...
          <el-form ref="feDeviceAddress" :model="feDeviceAddress" :rules="feDeviceAddresRules" label-width="100px" size="mini" @submit.native.prevent>
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="设备描述" prop="deviceDesc">
                  <el-col :span="21" style="margin-left: 0px">
                    <el-input v-model.trim="feDeviceAddress.deviceDesc" :maxlength="20" type="textarea" :rows="5" />
                  </el-col>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <el-dialog title="批量迁移设备" :visible.sync="OpenPasteDialogVisible" :close-on-click-modal="false" @close="closePasteDialog">
        <div>
          <el-form ref="pasteDeviceEditForm" :model="pasteDeviceEditForm" :rules="pasteDeviceEditFormRule" label-width="100px" size="mini" @submit.native.prevent>
            <el-row>
              <el-col :span="20">
                <el-form-item label="网关名称" prop="edgeDeviceId">
                  <el-col :span="21">
                    <el-select v-model.trim="pasteDeviceEditForm.edgeDeviceId">
                      <el-option v-for="item in feEdgeList" :key="item.edgeDeviceId" :label="item.edgeDeviceName" :value="item.edgeDeviceId" />
                    </el-select>
                  </el-col>
                </el-form-item>
              </el-col>
        ...
        <div slot="footer">
          <el-button icon="fa-ban" @click="closePasteDialog">取 消</el-button>
          <el-button type="primary" icon="fa-save" :loading="submiting" @click="savePasteDevice">
            确 定</el-button>
        </div>
      </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      type: '',
      deledata: [],
      OpenPasteDialogVisible: false,
      pasteDeviceEditForm: {
        edgeDeviceId: ''
      },
      pasteDeviceEditFormRule: {
        edgeDeviceId: [
          {
            required: true,
            message: '请选择网关名称!',
      ...
      feEdgeDeviceEditDesc: {
        nid: '',
        deviceDesc: ''
      },
      copyDeviceEditFormDesc: {
        deviceDesc: [
          {
            required: true,
            message: '请输入设备描述!',
  ...
  methods: {
    handlePartnerCmd(cmd, row) {
      ...
      } else if (cmd === 'updateDesc') {
        this.changeAdminDesc(row)
      }
    },
    changeAdminDesc(row) {
      this.feEdgeDeviceEditDesc.deviceDesc = row.deviceDesc
      this.feEdgeDeviceEditDesc.nid = row.nid
      this.editDialogTitle = '修改设备描述'
      this.editDialogDesc = true
      this.$nextTick(() => {
        this.feEdgeDeviceEditDesc.deviceDesc = row.deviceDesc
      })
    },
    ...
    returnQuery() {
      this.$router.push(
        `/config?edgeDeviceId=${this.listQuery.agwId}&type=${this.type}&deviceName=${this.listQuery.deviceName}&connectProtocol=${this.listQuery.connectProtocol}&deviceStatus=${this.listQuery.deviceStatus}&runOnce=${this.listQuery.runOnce}&currentPage=${this.listQuery.currentPage}&pageSize=${this.listQuery.pageSize}`
      )
    },
    Query() {
      ...
      this.listQuery.runOnce = this.$route.query.runOnce || ''
    ...
    handleSelectionChange(val) {
      ...
      if (this.length === 1) {
        ...
        this.Nid.nid = this.idList.toString()
        this.Nids.deviceIds = this.idList.toString()
      }
      if (this.length > 1) {
        this.abled = false
        this.disabled = true
    ...
    openPartnerEditDialog(actionType, row) {
      ...
      if (this.currentActionType === 'add') {
        ...
        this.$nextTick(() => {
          ...
          this.feDeviceAddressEditForm.deviceId = row.deviceId
          this.feDeviceAddressEditForm.deviceName = row.deviceName
          this.feDeviceAddressEditForm.addrId = row.addrId
          this.feDeviceAddressEditForm.addrProtocol = row.addrProtocol
          this.feDeviceAddressEditForm.address = row.address
          this.feDeviceAddressEditForm.port = row.port
    ...
    closeEditDialogDesc() {
      this.editDialogDesc = false
      this.$refs.feEdgeDeviceEditDesc.resetFields()
      this.feEdgeDeviceEditDesc.deviceDesc = ''
      this.multipleSelection = []
    },
    saveFeDeviceDesc() {
      this.$refs['feEdgeDeviceEditDesc'].validate((valid) => {
        if (valid) {
          saveFeDeviceDesc(this.feEdgeDeviceEditDesc)
            .then((response) => {
              if (response.success) {
                this.closeEditDialogDesc()
                this.getList()
                this.$notify({
                  message: '保存成功!',
                  type: 'success',
    ...
    openDataView(params) {
      console.log(123123);
      var keyword = encodeURIComponent(params.deviceName)
      console.log(params)
      console.log(keyword)
      
      this.$router.push(
        `/openDataView?agwId=${params.agwId}&deviceName=${keyword}&profileNid=${params.profileNid}`
      )
    },
    ...
    closeCopyDialog() {
      ...
      this.copyDeviceEditForm.deviceDesc = ''
    },
    closeEditDialogmode() {
    ...
    deleteAll() {
      this.$confirm('请确认是否批量删除记录?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          deletePartners(this.Nid)
            .then((response) => {
              if (response.success) {
                this.$notify({
                  message: '删除成功!',
    ...
    queryList() {
      this.edw.edgeDeviceId = this.edgeDeviceId
      findByAgwId(this.edw)
        .then((response) => {
          if (response.success) {
            this.feEdgeList = response.data
          } else {
            this.feEdgeList = []
          }
        })
    ...
    pasteAll() {
      this.queryList()
      this.OpenPasteDialogVisible = true
    },
    closePasteDialog() {
      this.OpenPasteDialogVisible = false
      this.$refs.pasteDeviceEditForm.resetFields()
      this.pasteDeviceEditForm.edgeDeviceId = ''
      this.multipleSelection = []
    },
    savePasteDevice() {
      this.$confirm(
        '该批设备的相关告警、运行时长信息将被同时删除，请确认是否迁移此批设备？',
        '提示',
        {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        ...
        this.Nids.edgeDeviceId = this.pasteDeviceEditForm.edgeDeviceId

        pasteDevice(this.Nids)
          .then((response) => {
            if (response.success) {
              this.$notify({
                message: ' 批量迁移设备成功!',
