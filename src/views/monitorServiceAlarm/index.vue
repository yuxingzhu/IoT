<template>
  <div class="app-container">
      ...
      <el-row>
        <el-col :span="24" class="table-filter">
          <el-form ref="filterForm" :inline="true" :model="listQuery" label-width="0px" class="filterForm" @submit.native.prevent>
            ...
            <el-form-item>
              <el-form-item prop="edgeDeviceNameP">
                <el-input v-model.trim="listQuery.edgeDeviceNameP" placeholder="网关告警名称"></el-input>
            </el-form-item>
    ...
    <el-table v-loading="listLoading" :data="list" stripe size="mini">
      ...
      <el-table-column label="服务别名" prop="serviceAliasName" align="center" />
      ...
      <div v-if="listQuery.alarmStatus ==='false'" >
        <el-table-column label="告警结束时间" prop="alarmEndTime" align="center" />
      </div>
...
<script>
import { getList } from '@/api/monitorServiceAlarm'
  ...
  methods: {
    ...
    exportExcel(arr) {
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, "服务告警"); //最后一个是表名字
      });
    },
