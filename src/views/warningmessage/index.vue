<template>
    <div class="app-container">
          ...
          <el-col :span="20" class="table-filter">
            <el-form ref="filterForm" :inline="true" :model="listQuery" label-width="0px" class="filterForm" @submit.native.prevent>
              ...
              <el-form-item prop="bizType">
                <el-select v-model.trim="listQuery.bizType" placeholder="业务类型">
                  <el-option v-for="item in monitorList" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
              </el-form-item>
              <!-- <el-form-item prop="dataAlarmStartTime">
                <el-date-picker v-model.trim="listQuery.dataAlarmStartTime" :editable="false" :clearable="false" type="datetimerange" start-placeholder="告警开始时间" end-placeholder="告警开始时间" value-format="yyyy-MM-dd HH:mm:ss" />
              </el-form-item> -->
                       <el-form-item prop="dataAlarmStartTime">
                <el-date-picker v-model.trim="listQuery.dataAlarmStartTime" :editable="false" :clearable="false" type="date" placeholder="告警开始时间"  value-format="yyyy-MM-dd HH:mm:ss" />
              </el-form-item>
                       <el-form-item prop="dataAlarmEndTime">
                <el-date-picker v-model.trim="listQuery.dataAlarmEndTime" :editable="false" :clearable="false" type="date"   placeholder="告警结束时间" value-format="yyyy-MM-dd HH:mm:ss" />
              </el-form-item>
      ...
      <el-table v-loading="listLoading" :data="list" stripe size="mini">
        ...
        <el-table-column label="告警项名称" prop="dataAlarmName" align="center"></el-table-column>
        <el-table-column label="告警设备" prop="deviceName" align="center"></el-table-column>
        <el-table-column label="告警信息" prop="dataAlarmInfo" align="center"></el-table-column>
        <el-table-column label="告警级别" prop="alarmLevel" align="center"></el-table-column>
        <el-table-column label="告警时间" prop="dataAlarmStartTime" align="center"></el-table-column>
        <el-table-column label="处理状态" prop="dealStatus" align="center">
          <template slot-scope="scope">
            ...
            <span>{{scope.row.dealStatus==true?"已处理":"未处理"}}</span>
          </template>
        </el-table-column>
          ...
          <!-- <el-table-column label="操作" align="center" >
            <template slot-scope="scope">
              <el-tooltip content="处理">
                <el-button class="elbuttondefault" 
  ...
  import { getList,deleteAlarmData,dealAlarmData} from '@/api/warningmessage'
    ...
    methods: {
      ...
      exportExcel(arr) {
        require.ensure([], () => {
          ...
          export_json_to_excel(tHeader, data, "数据告警管理"); //最后一个是表名字
        });
      },
      ...
      updateData() {
        this.$refs['dataForm'].validate(valid => {
          if (valid) {
            dealAlarmData(this.temp).then(response => {
              this.getList()
