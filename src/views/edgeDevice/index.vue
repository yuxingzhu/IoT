        ...
        <el-row>
          <el-col :span="24" class="table-filter">
            ...
            >
              ...
              <el-form-item>
                <el-select
                  v-model.trim="listQuery.gatewayId"
                  style="width: 100%"
                  placeholder="网关型号"
                >
                  <el-option
              ...
              <!-- <el-tooltip content="导出">
                <el-button icon="el-icon-download" @click="onload1">导出</el-button>
              </el-tooltip> -->
            </el-form>
          </el-col>
        </el-row>
      ...
      >
        ...
        <el-table-column label="网关告警" prop="gatewayAlarms" align="center">
          <template scope="scope">
            <el-button
              :type="
                scope.row.gatewayAlarms == '警告'
                  ? 'danger'
                  : scope.row.gatewayAlarms == '一般严重'
                  ? 'danger'
                  : scope.row.gatewayAlarms == '灾难'
                  ? 'danger'
                  : scope.row.gatewayAlarms == '未分类'
                  ? 'success'
                  : scope.row.gatewayAlarms == '咨询'
                  ? 'success'
                  : scope.row.gatewayAlarms == '正常'
                  ? 'success'
                  : 'info'
              "
              circle
              style="border-radius: 10px"
              @click="onloaddata(scope.$index, scope.row)"
            >
              {{
        ...
        <el-table-column label="状态" prop="status" align="center">
          <template scope="scope">
            <svg-icon :icon-class="scope.row.status === true ? 'online' : 'offline'" />
            {{ scope.row.status === true ? "在线" : "离线" }}
          </template>
        </el-table-column>
        <el-table-column label="网关型号" prop="gatewayId" align="center">
          <template scope="scope">
            ...
            {{
              scope.row.gatewayId===8?"XUN2000" 
              :scope.row.gatewayId===9
              ? "XUN3000"
              : scope.row.gatewayId===11
                ? "RTY"
                : 10
            }}
          </template>
      ...
      >
        <div>
          ...
          >
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="网关类型" prop="gatewayCategory">
                  <el-col :span="22">
                    <el-select
                      v-model.trim="feEdgeDeviceEditForm.gatewayCategory"
                      placeholder="请选择网关类型"
                    >
                      <el-option
                        v-for="item in gatewayCategory"
                        :key="item"
                        :label="item"
                        :value="item"
                      />
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="设备SN" prop="deviceSn">
                  <el-col :span="22">
                    <el-input v-model.trim="feEdgeDeviceEditForm.deviceSn"  placeholder="请输入设备SN码"/>
                  </el-col>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row v-if="feEdgeDeviceEditForm.gatewayCategory==='工业级DTU'">
              <el-col :span="20" >
                <el-form-item label="新增sim卡IP" prop="simIpAddress">
                  <el-col :span="22">
                    <el-input
                      v-model.trim="feEdgeDeviceEditForm.simIpAddress"
                      :maxlength="10000"
                      placeholder="请输入SIM卡号"
                    />
                  </el-col>
            ...
            <el-row>
              <el-col :span="20">
                <el-form-item label="url" prop="url">
                    <el-col :span="22">
                    <el-input
                      v-model.trim="feEdgeDeviceEditForm.urlAddress"
                      placeholder="请输入url"
                    />
                  </el-col>
                </el-form-item>
            ...
            <el-col :span="24">
              ...
              >
                ...
                <el-row style="margin-top: 10px">
                  ...
                  <el-col :span="14">{{ detailForm.simIpAddress }}</el-col>
                </el-row>
                <el-row style="margin-top: 10px">
...
export default {
  ...
  data() {
    return {
      ...
      gatewayCategory: ["边缘计算网关", "工业级DTU"],
      devicePosition: ["弹簧车间", "组装车间", "切割车间", "海绵车间"],
      feEdgedata:{
        projectId:[ {
            required: true,
            message: "请选择所属项目!",
      ...
      feDeviceAddressEditRules: {
        ...
        simIpAddress:[
         ...
         {
            validator: (rule, value, callback) => {
              if (!/^([1-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(\.([1-9]?[0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])){3}$/.test(value)) {
                callback("sim卡的地址格式为(1-255).(0-255).(0-255).(0-255)形式！");
              }
              callback();
            },
            trigger: "blur",
  ...
  created() {
    ...
    this.feEdgeDeviceStatus(this.getList);
    this.initCityData();
    this.getAll();
  },
  activated() {
  ...
  methods: {
    ...
    queryByPage() {
      ...
      getList({
        edgeDeviceName: this.listQuery.edgeDeviceName,
        area: this.listQuery.area,
        agwId: this.listQuery.agwId,
        ...
        gatewayId: this.listQuery.gatewayId,
        ...
        if (response.success) {
          ...
          for (let i = 0; i < newData.length; i++) {
            for (let j = 0; j < deviceList.length; j++) {
              if(newData[i].deviceSn == deviceList[j].deviceId) {
                newData[i].status = deviceList[j].status
              }
            }
          }
          if (Number(this.listQuery.status) === 1) {
            this.list = newData.filter(item=>item.status === true).slice(start, end)
            this.total = newData.filter(item=>item.status === true).length;
          } else if (Number(this.listQuery.status) === 2) {
            this.list = newData.filter(item=>item.status !== true).slice(start, end)
            this.total = newData.filter(item=>item.status !== true).length;
          }
    ...
    async feEdgeDeviceStatus(cb) {
      let deviceList = [];
      try {
        await feEdgeDeviceStatus().then(response=>{
          deviceList = response && (response.deviceData && response.deviceData.length)
           ? response.deviceData : []
        })
      } catch(err) {
        deviceList = []
    ...
    onloaddata(index, row) {
      this.$router.push({
        path: "monitorGw",
        query: { edgeDeviceName: row.edgeDeviceName },
      });
    },
    onload() {
      ...
      getList(this.listQuery).then((response) => {
        ...
        if (response.success) {
          ...
          for (let i = 0; i < arrs.length; i++) {
            for (let j = 0; j < deviceList.length; j++) {
              if(arrs[i].deviceSn == deviceList[j].deviceId) {
                arrs[i].status = deviceList[j].status
              }
            }
          }
          if (Number(this.listQuery.status) === 1) {
            arrs = arrs.filter(item => item.status === true)
          } else if (Number(this.listQuery.status) === 2) {
            arrs = arrs.filter(item => item.status !== true)
          }
          arrs.map((el, index) => {
    ...
    uploadFile() {
      ...
      batchimport(formDataInfo).then((res) => {
        ...
        this.feEdgeForm.gatewayId="",
        this.feEdgeForm.projectId="",
        this.feEdgeForm.area="",
        this.fileList=[],
    ...
    dictionarysort() {
      ...
      this.listLoading = true;
      dictionarysort(this.listQuery).then((response) => {
        if (response.success) {
          let newData = response.data;
          let deviceList = this.deviceList
          for (let i = 0; i < newData.length; i++) {
          ...
          if (Number(this.listQuery.status) === 1 || Number(this.listQuery.status) === 2) {
            this.queryByPage()
          } else {
            this.list = newData;
      ...
      getAll()
        .then((response) => {
          if (response.success) {
            ...
            console.log( response.data,"343")
            console.log(this.gatawayList,"34234")
          } else {
            this.gatawayList = [];
          }
        })
    ...
    closeEditDialog() {
      ...
      this.feEdgeDeviceEditForm.urlAddress = "";
      this.areaList = [];
    ...
    openPartnerEditDialog(actionType, row) {
      ...
      if (this.currentActionType === "add") {
        ...
        this.$nextTick(() => {
          this.feEdgeDeviceEditForm.simIpAddress=row.simIpAddress
          ...
          this.feEdgeDeviceEditForm.gatewayCategory = row.gatewayCategory;
          this.feEdgeDeviceEditForm.deviceLocation = row.deviceLocation;
          this.feEdgeDeviceEditForm.urlAddress = row.urlAddress;
    ...
    handlePartnerCmd(cmd, row) {
      if (row.gatewayCategory == "工业级DTU") {
        // window.open(`http://${row.simIpAddress}:11001`)
        if(row.urlAddress) {
    ...
    openDataView(row) {
      this.detailForm = {
        ...
        gatewayCategory: row.gatewayCategory,
...
::v-deep .el-table .el-table__body td .cell > span:empty::after,
::v-deep .el-table .el-table__body td .cell .approval:empty::after,
::v-deep .el-table .el-table__body td .cell div:empty::after,
::v-deep .el-table .el-table__body td .cell .line-limit-length:empty::after {
  content: '--';
}
</style>
