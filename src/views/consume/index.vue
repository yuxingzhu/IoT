          ...
          <el-row>
            <el-col>
              <el-form-item label="主题名称" prop="topicName">
                <el-input v-model.trim="temp.topicName"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
<script>
import { getList, createData, updateData, deleteData } from '@/api/consume'
...
export default {
  ...
  methods: {
    ...
    handleView(row) {
      this.$router.push(
        `/consumeConfigViews?collectionName=${row.dataTableName}`
      )
    }
  }
}
